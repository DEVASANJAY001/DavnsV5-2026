...
export function GlassmorphismNav() {
  ...
  const scrollToSection = (e: React.MouseEvent, href: string) => {
    ...
    if (pathname !== "/") {
      setIsOpen(false)
      // If we're not on the home page, let the default Link behavior handle it
      // or manually push if it's a button logic. 
      // Actually, since we want smooth scroll on the home page, we use this logic.
      return
    }
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="hidden md:flex items-center space-x-8">
                {navigation.map((item) => {
                  ...
                  const fullHref = isHashLink ? `/${item.href}` : item.href

                  return (
                    <Link
                      key={item.name}
                      href={fullHref}
                      onClick={(e) => isHashLink && scrollToSection(e, item.href)}
                ...
                {navigation.map((item, index) => {
                  ...
                  return (
                    <Link
                      ...
                      href={fullHref}
                      onClick={(e) => {
                        if (isHashLink) scrollToSection(e, item.href)
